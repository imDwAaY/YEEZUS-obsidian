...
```
假设有一个大小为`n`的词典，对于每一封邮件提取一个特征向量$\begin{align*}F \in \mathbb{R}^n,\end{align*}$我们把第`i`个特征`F_i`当成一个bool值的随机变量。也就是说值为1就代表第i个词出现了，值为0则代表没有出现。如果第200个词为free并且`F_200 = 1`则代表这封邮件里面出现了free。
